...
The expression sizeof(int) calculates the number of bytes required to store an integer on the system, and the cast (int*) is used to convert the void pointer returned by malloc() into a pointer of type int.
...
Finally, the program prints out the value of the integer using the expression printf("The value of the integer is %d", *ptr);, 
